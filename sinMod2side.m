...
multipleFo=(0:fo:fs);       % odczytanie wielokrotnosci fo
FoHop=floor(length(P2)/length(multipleFo)); % wyliczenie co ile probek bedziemy odczytywac amplitude i faze

A=zeros(1,length(multipleFo));  %uwtorzenie zerowego wektora amplitud
j=0;
for i=2:FoHop:length(P2)
    j=j+1;
    A(j)=P2(i);                 %przypisanie wartosci amplitud
end

fi=zeros(1,length(multipleFo));  %uwtorzenie zerowego wektora amplitud
j=0;
for i=2:FoHop:length(phs2)
    j=j+1;
    fi(j)=phs2(i); %przypisanie wartosci fazy
end
...
xx=zeros(1,(length(f)*2)-1);  % wektor dlugosci naszego sygnalu
j=1;
for i=1:ceil((length(f)*2)/length(A)):(length(f)*2)-1 
